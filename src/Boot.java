...
public class Boot {
	...
	public static void main(String[] args) throws IOException {
		...
		File file = new File("H:\\test.jpg");
		byte[] data = new String("第一回　甄士隐梦幻识通灵　贾雨村风尘怀闺秀\r\n" + 
				...
				"　　这日，那甄家大丫鬟在门前买线，忽听街上喝道之声，众人都说新太爷到任。丫鬟于是隐在门内看时，只见军牢快手，一对一对的过去，俄而大轿抬着一个乌帽猩袍的官府过去。丫鬟倒发了个怔，自思这官好面善，倒像在那里见过的。于是进入房中，也就丢过不在心上。至晚间，正待歇息之时，忽听一片声打的门响，许多人乱嚷，说：“本府太爷差人来传人问话。”封肃听了，唬得目瞪口呆，不知有何祸事。 ").getBytes();
		//File df = new File("H:\\Okami_Yuuki.txt");
		//int length = (int) df.length();
		//byte[] data = new byte[length];
		//new FileInputStream(df).read(data);
		System.out.println("Size:"+data.length);
		System.out.println(Arrays.toString(data));
		ImageIO.write(ByteFour.draw(data, 2000, 4,4), "JPEG",file);
		data = ByteFour.deposit(ImageIO.read(file), 4, 4);
		System.out.println("Size:"+data.length);
		System.out.println(Arrays.toString(data));
